...
class PythiaLinear(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        if self._use_flash_attention_2:
            ...
            attention_mask = attention_mask.to(self.final_transformer_layer.attention.dense.weight.dtype)  # fp16 compatibility
            attention_mask = (1.0 - attention_mask) * torch.finfo(self.final_transformer_layer.attention.dense.weight.dtype).min
    ...
    def from_pretrained(pretrained_mamba_name: str, pretrained_pythia_name: str):
        ...
        model = PythiaLinear(args).to('cuda')
    ...
    def freeze_layers_except_mamba(self):
        ...
        # Directly using a forward layer
        for layer in self.mamba_layers:
...
class PythiaLinearForLM(PreTrainedModel):
    def __init__(self, config=None, check_point_path=None, distilling=None, T=4, distill_loss_weight=0.75):
        ...
        self.model = PythiaLinear.from_pretrained(pretrained_mamba_name, pretrained_pythia_name)
...
class PythiaLinearConfig(PretrainedConfig):
    def __init__(
        self,
        **kwargs,
    ):
